...
function updateTotals() {
    try {
        ...
        expensesQuantity.innerHTML = `${items.length} ${items.length <= 1 ? 'despesa' : 'despesas'}`
        ...
        for (let item = 0; item < items.length; item++) {
            ...
            if(isNaN(value)) {
                return alert("Não foi possível localizar o total, o valor não parece ser um número")
            }
...
expenseList.addEventListener("click", (ev) => {
    if(ev.target.classList.contains('remove-icon')) {
        const item = ev.target.closest(".expense");
